...
let OnboardingService = OnboardingService_1 = class OnboardingService {
        ...
        if (dto.settings) {
            ...
            await this.prisma.workflow.update({
                ...
                data: {
                    ...
                    formHeaderRich: dto.settings.formHeaderRich === undefined
                        ? undefined
                        : dto.settings.formHeaderRich || client_1.Prisma.JsonNull,
                    showFormHeader: dto.settings.showFormHeader ?? true,
                    formDescription: dto.settings.formDescription || null,
                    formDescriptionRich: dto.settings.formDescriptionRich === undefined
                        ? undefined
                        : dto.settings.formDescriptionRich || client_1.Prisma.JsonNull,
                    showFormDescription: dto.settings.showFormDescription ?? true,
                    settings: {
                        ...workflowSettings,
                        form: {
                            ...(workflowSettings.form || {}),
                            submitButtonText: dto.settings.submitButtonText || 'Submit',
                            successMessage: dto.settings.successMessage ||
    ...
    async getFormFields(userId, workflowId) {
        ...
        const availableVariables = allVariables;
        const workflowSettings = workflow.settings?.form || {};
        const formSettings = {
            ...
            successMessage: workflowSettings.successMessage ||
                "Thank you! We'll be in touch soon.",
    ...
    async saveCalendlyLink(userId, dto) {
        ...
        await this.oauthService.saveCalendlyLink(userId, workflow.workspaceId, dto.calendlyLink);
        await this.prisma.workflow.update({
            where: { id: dto.workflowId },
            data: {
    ...
    async saveConfiguration(userId, dto) {
        ...
        if (workflow) {
            ...
            this.logger.log(`Saved configuration to workflow ${workflow.id}`);
        }
        return {
            success: true,
            data: {
    ...
    async activateWorkflow(userId, dto) {
        const session = await this.prisma.onboardingSession.findUnique({
            where: { id: dto.configurationId },
            include: {
                workspace: true,
        ...
        if (formFieldCount < 2) {
            throw new common_1.BadRequestException('Workflow must have at least 2 form fields (name and email)');
        }
        if (!session.configurationData) {
            throw new common_1.BadRequestException('Configuration data is missing. Please complete configuration step.');
        }
        ...
        this.logger.log(`Workflow ${result.workflow.id} activated for user ${userId}`);
        return {
            success: true,
            data: {
